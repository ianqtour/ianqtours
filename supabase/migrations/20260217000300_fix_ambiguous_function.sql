...
DROP FUNCTION IF EXISTS get_financial_dashboard_metrics(INT, INT);
DROP FUNCTION IF EXISTS get_financial_dashboard_metrics(UUID, UUID);

-- Recreate the correct function with UUID parameters
CREATE OR REPLACE FUNCTION get_financial_dashboard_metrics(
  p_excursao_id UUID DEFAULT NULL,
